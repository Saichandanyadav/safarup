...
export default function Connect() {
  ...
  const SwipeCard = ({ person }: { person: Person }) => {
    ...
    return (
      ...
      >
        <motion.div style={{ opacity: rightTextOpacity }} className="absolute inset-0 flex items-center justify-center text-5xl font-black text-emerald-400 z-30 pointer-events-none">ACCEPT</motion.div>
        <motion.div style={{ opacity: leftTextOpacity }} className="absolute inset-0 flex items-center justify-center text-5xl font-black text-rose-500 z-30 pointer-events-none">DECLINE</motion.div>
        <motion.div style={{ opacity: downTextOpacity }} className="absolute inset-0 flex items-center justify-center text-5xl font-black text-sky-400 z-30 pointer-events-none">SKIP</motion.div>
        ...
        <div>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {person.interests.map((interest, index) => (
              <span key={index} className="px-3 py-1 bg-white/10 border border-white/10 text-xs rounded-full">{interest}</span>
            ))}
          </div>
        </div>
        ...
        <div className="flex justify-center gap-8">
          ...
          <button onClick={() => handleSkip(person)} className="w-14 h-14 rounded-full bg-white/5 border border-white/20 flex items-center justify-center text-sky-400 text-xl font-bold active:scale-95 transition">↓</button>
      ...
      <div className="relative h-[45vh] sm:h-[55vh] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
          <h1 className="text-3xl sm:text-5xl font-extrabold leading-tight">
            Connect with Fellow
            <span className="block bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">Travelers</span>
          </h1>
        </div>
      </div>
      ...
      <div className="px-5 sm:px-6 py-12 max-w-7xl mx-auto">
          ...
          <>
            <div className="sticky top-0 z-20 bg-slate-950/80 backdrop-blur-xl border-b border-white/10 mb-8">
              <div className="flex justify-between text-sm font-semibold">
                <button onClick={() => setActiveTab("declined")} className={`flex-1 py-4 ${activeTab==="declined"?"text-rose-500 border-b-2 border-rose-500":"text-gray-400"}`}>Declined ({declined.length})</button>
                <button onClick={() => setActiveTab("current")} className={`flex-1 py-4 ${activeTab==="current"?"text-cyan-400 border-b-2 border-cyan-400":"text-gray-400"}`}>Current ({requests.length})</button>
                <button onClick={() => setActiveTab("accepted")} className={`flex-1 py-4 ${activeTab==="accepted"?"text-emerald-400 border-b-2 border-emerald-400":"text-gray-400"}`}>Accepted ({accepted.length})</button>
              </div>
            </div>
            ...
            {activeTab === "current" && (
              <div className="flex justify-center relative h-[520px]">
                <AnimatePresence mode="wait">
                  {requests.length > 0 ? <SwipeCard key={requests[0].id} person={requests[0]} /> : <p className="text-gray-500 mt-20">No current requests</p>}
                </AnimatePresence>
              </div>
            )}
            ...
            {activeTab === "accepted" && (
              <div className="space-y-4">
                {accepted.length > 0 ? accepted.map((p) => <ListCard key={p.id} person={p} />) : <p className="text-gray-500 text-center">No accepted requests</p>}
              </div>
            )}

            {activeTab === "declined" && (
              <div className="space-y-4">
                {declined.length > 0 ? declined.map((p) => <ListCard key={p.id} person={p} />) : <p className="text-gray-500 text-center">No declined requests</p>}
              </div>
            )}
          </>
        )}
        ...
        {isLarge && (
          <div className="grid grid-cols-3 gap-10">
            <div>
              <h3 className="text-xl font-bold text-rose-500 mb-6 text-center">Declined ({declined.length})</h3>
              <div className="space-y-4">{declined.length > 0 ? declined.map((p) => <ListCard key={p.id} person={p} />) : <p className="text-gray-500 text-center">No declined requests</p>}</div>
            </div>

            <div className="flex flex-col items-center">
              <h3 className="text-xl font-bold text-cyan-400 mb-6 text-center">Current ({requests.length})</h3>
              <div className="flex justify-center relative h-[520px] w-full max-w-sm">
                <AnimatePresence mode="wait">{requests.length > 0 ? <SwipeCard key={requests[0].id} person={requests[0]} /> : <p className="text-gray-500 mt-20">No current requests</p>}</AnimatePresence>
              </div>
            </div>

            <div>
              <h3 className="text-xl font-bold text-emerald-400 mb-6 text-center">Accepted ({accepted.length})</h3>
              <div className="space-y-4">{accepted.length > 0 ? accepted.map((p) => <ListCard key={p.id} person={p} />) : <p className="text-gray-500 text-center">No accepted requests</p>}</div>
            </div>
          </div>
        )}
      </div>
